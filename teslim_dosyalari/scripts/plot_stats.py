...
def plot_distributions(csv_input, output_plot, output_stats):
    ...
    # 1. Özet İstatistikleri Hesaplama ve Yazdırma (Mean & Median)
    stats_df = df[['GC_Content_Percent', 'Read_Length', 'Mean_Quality']].agg(['mean', 'median']).round(2)
    print("--- Temel Özet İstatistikler ---")
    print(stats_df)
    
    # İstatistikleri profesyonelce raporlamak için ayrı bir metin dosyasına da kaydedelim
    stats_df.to_csv(output_stats)

    # 2. Görselleştirme (Grafik Çizimi)
    # Seaborn ile profesyonel bir tema ayarlıyoruz
    sns.set_theme(style="whitegrid")
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    
    # A. GC İçeriği Dağılımı (Histogram + Yoğunluk)
    ...
    # C. Ortalama Kalite Skoru Dağılımı
